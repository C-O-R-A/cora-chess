...
def TF(pos_vec, rot_vec):
    ...
    TF = np.array([
        ...
        [C_x*S_z + C_z*S_x*S_y,      C_x*C_z - S_x*S_y*S_z,     -C_y*S_x,     pos_vec[1]],
        [S_x*S_z - C_x*C_z*S_y,      C_z*S_x + C_x*S_y*S_z,      C_y*C_x,     pos_vec[2]],
        [0,                          0,                                0,              1]
        ])
...
async def move_to_square(target_sq, height, board, robot_client, print_status=False):
    ...
    await robot_client.send_command(target, space='TS', rt=False, interface_type='position', print=False)
